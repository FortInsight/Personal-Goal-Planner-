...
function initialize() {
  ...
  render({ skipSave: true });
  queueMicrotask(() => {
    if (state.ui.activeView === "home" && elements.homeActionMode.value === "create") {
      focusGoalTitleField();
    }
  });
}
...
function registerPwa() {
  ...
  window.addEventListener("load", async () => {
    try {
      ...
      console.error("Planner service worker cleanup failed.", error);
    }
  });
}
        ...
        const suffix =
          goal.repeat && goal.repeat !== "none"
            ...
            : goal.dueDate
              ...
              : "";
        return `<option value="${goal.id}">${escapeHtml(goal.title)}${suffix}</option>`;
      }),
    )
    .join("");
